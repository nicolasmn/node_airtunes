...
function AirTunesDevice(host, options) {
  ...
  this.dacp = new DACP.Server(options.dacpPort || 3689, this.rtsp.dacpId, this.rtsp.activeRemote);
...
AirTunesDevice.prototype.start = function() {
  ...
  this.dacp.on('command', function(cmd, args) {
    self.emit('remote-command', cmd, args);
  });
  this.dacp.on('error', function(error) {
    self.emit('error', 'dacp-server', error);
  });
  this.dacp.start();
};
